...
print(
    f"Duplicate Rows except first occurrence based on all columns are :\n{duplicateRowsDF}"
)
...
indexNames = globe_df[
    (globe_df["School Name"] != "United States of America Citizen Science")
].index
...
for k in rand_range:
    x_train, x_test, y_train, y_test = train_test_split(
        x, y, test_size=0.20, random_state=k
    )
...
x_train, x_test, y_train, y_test, indx_train, indx_test = train_test_split(
    x, y, indices, test_size=0.20, random_state=rand_st
)
...
compare_df = pd.DataFrame(
    {
        ...
        "Predicted": y_pred,
        "Residuals": residuals,
        "Absolute Val of Residuals": abs(residuals),
    }
)
...
rgb_and_pred_ndvi = rgb_and_pred_ndvi[
...
fig = plt.figure(figsize=(10, 6))
plt.scatter(
    compare_df.index, compare_df["Actual"], color="blue", marker="o", label="Actual"
)
plt.scatter(
    compare_df.index,
    compare_df["Predicted"],
    color="orange",
    marker="*",
    label="Predicted",
)
...
def get_img_from_resid(target_resid):
    pic_loc = compare_df[
        (compare_df["Absolute Val of Residuals"] == target_resid)
    ].index
    pic_loc = pic_loc[0]
